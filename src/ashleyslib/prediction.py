...
def add_prediction_parser(subparsers):
    ...
    parser.add_argument(
        "--path",
        "-p",
        help="path to feature table of data that should be predicted",
        required=False,
    )
    ...
    parser_group.add_argument(
        "--annotation", "-a", help="path to folder with annotation files", required=False
    )

    parser_group_comp = parser.add_argument_group("Compare two predictions")
    parser_group_comp.add_argument(
        "--prediction_1", "-p1", help="prediction file for first model", required=False
    )
    parser_group_comp.add_argument(
        "--prediction_2", "-p2", help="prediction file for second model", required=False
    )
            ...
            if len(w) != 0:
                for message in w:
                    ...
                    if len(versions) > 2 and issubclass(w[-1].category, UserWarning):
                        warn_message = (
                            "You are using a different version of scikit-learn than the one used for "
                        ...
                        warn_message += (
                            "The model was trained with scikit-learn version "
                            + version_model
                            + "while you have version "
                            + version_installed
                            + " installed.\n"
...
def run_prediction(args):
    ...
    if args.prediction_1 is not None and args.prediction_2 is not None:
        logger.info("comparing two predictions of different models")
    ...
    logger.info(
        "predicting class probabilities, extracting critical predictions between bound "
        + str(critical_bound)
    )
    prediction, probability = predict_model(args.model, features)

    if args.annotation is not None:
        logger.info("comparing prediction to given annotation")
    ...
    for i in range(len(names)):
        pred_file.write(
            names[i] + "\t" + str(prediction[i]) + "\t" + str(round(probability[i], 4)) + "\n"
        )
